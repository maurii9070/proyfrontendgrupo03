...
export const roleGuard: CanActivateFn = (route, state) => {
  ...
  if (!expectedRole) {
    console.error('RoleGuard: No se especificó un rol esperado en la configuración de la ruta.');
  ...
  return authService.currentUserProfile$.pipe(
    filter(profile => profile !== null), // Esperar hasta que el perfil no sea null
    ...
    map(profile => {
      ...
      if (profile._rol === expectedRole) {
        ...
        console.warn(`RoleGuard: Acceso denegado. Rol '${profile._rol}' no coincide con el rol requerido '${expectedRole}'.`);
